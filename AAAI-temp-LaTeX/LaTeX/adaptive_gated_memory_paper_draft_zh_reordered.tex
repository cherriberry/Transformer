...
为避免将不同任务下的数值误读为同一排行榜，本节先给出独立的背景筛选。第一层是在
...
本节结果来自独立的 TinyStories 语言建模筛选，用于提供注意力路径的背景参照；它们不使用
...
具体生命周期包含五个步骤。首先，用户或系统输入打开新一轮，读取器在最后一个提示词元处定位相关事实；该位置的输出概率用于预测第一个助手词元，因此长期记忆可以影响回答开头。其次，抽取器在用户词元中定位候选事实片段，并将其写入有界候选缓冲，但此时不改变可读槽位。第三，助手生成过程中复用同一组检索结果，避免词元间路由抖动，并通过运行时聚合和计数器累计本轮助手摘要。第四，到达提交边界时，模型先计算并输出本轮最后一个词元，再根据写入门、保留门和淘汰规则更新事实槽位及助手槽位。最后，轮次关闭，下一轮才可以读取新提交的事实。
...
组织实验。首先，在同一个 TinyStories 初始化模型上比较三种不带显式事实槽位的条件：
\textsc{FA-task} 使用完整历史注意力，\textsc{SWA-task} 只保留汇聚词元和最近词元，Memformer
则通过循环隐状态跨分段传递信息。三者只使用相同的因果语言模型损失与答案损失。随后对
Memformer 的循环状态进行清零、交换和逐段重置，以检验答案是否真正依赖跨段状态。
再比较两种显式事实记忆条件 Fixed-LRU 和 Gated Memory；前者按最近最少使用规则写入和淘汰，
后者学习写入与保留门，因此需要额外的事实候选、写入、读取和保留监督。
...
颜色任务包含七个条件，并沿用以下正式名称：FA-task、SWA-task、Memformer、Fixed-LRU、
Gated Memory、Value-token Gated 以及 Fixed-LRU + alignment。前三者只训练因果语言模型和答案
损失；后四者使用事实候选、写入和读取等与其结构相匹配的监督。因此，表格提供同一数据生成
方式下的数值，但不把不同监督口径伪装成单一公平排名。Value-token Gated 是在 Gated Memory
上加入值片段和记忆到词元对齐，Fixed-LRU + alignment 是对应的固定规则版本。Memformer 不加入
重建损失或对齐路径。
